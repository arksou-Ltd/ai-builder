...
1. Given 后端 workspace 初始化完成  
   When 配置模块依赖关系  
   Then 依赖链满足：`app-api → common-kernel → arksou-kernel-framework[all] v0.3.4`  
   And `common-kernel` 通过 Git SSH 源引入 `arksou-kernel-framework[all]@v0.3.4`（`[tool.uv.sources]` + `tag = "v0.3.4"`）  
...
- [x] 依赖链配置与对齐（AC: 1）
  ...
  - [x] 校验 `backend/common-kernel/pyproject.toml`：
    ...
    - [x] `[tool.uv.sources].arksou-kernel-framework` 使用 Git SSH 源且 `tag = "v0.3.4"`
...
- 依赖链路（必须满足）：`app-api → common-kernel → arksou-kernel-framework[all] v0.3.4`。
- Git SSH 依赖注意事项（**本仓库最容易踩坑点**）：
  - `arksou-kernel-framework` 必须通过 Git SSH 源拉取：`ssh://git@github-arksou/arksou-Ltd/arksou-kernel-framework.git`（并固定 `tag = "v0.3.4"`）。
...
| 框架来源与版本 | `arksou-kernel-framework` 必须走 Git SSH 源且 `tag = "v0.3.4"`（禁止替换为 PyPI 或其他 tag） |
...
| 依赖/约束 | 要求 |
| --- | --- |
| `arksou-kernel-framework[all]` | Git SSH 源 + `tag = "v0.3.4"` |
...
- 2026-02-04: 执行依赖链校验 - 所有配置项已满足 AC 要求
  - `.python-version` = `3.12` ✓
  - `common-kernel` 通过 Git SSH 源依赖 `arksou-kernel-framework[all]@v0.3.4` ✓
...
- 2026-02-04: Code Review Round 3 修复:
  ...
  - [Medium] 修复 architecture.md 版本不一致（v0.3.1 → v0.3.4）
