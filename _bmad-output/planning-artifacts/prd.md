...
| 工作空间管理 | 创建工作空间并导入多个 GitHub 仓库（后端 + 多前端） |
...
2. 创建一个新工作空间"电商平台"，导入三个 GitHub 仓库：
   - 后端 Spring 仓库（仓库A）
   - Flutter 移动端仓库（仓库B）
   - Next.js Web端仓库（仓库C）
...
**Opening Scene（开场）：**
小李正在工作空间中实现一个"订单导出"功能，已经完成了后端 API 和 Flutter 端的展示。
...
| 旅程 | 揭示的能力需求 | 对应 FR |
|------|---------------|---------|
| PM 首次成功 | Clerk 登录 + GitHub OAuth 授权、工作空间管理（含多仓库导入）、AI 渠道配置、工作流引导、跨仓库需求实现、多 PR 提交 | FR1-FR19, FR24-FR38, FR42-FR56 |
...
| 合规维度 | MVP 状态 | 说明 |
|----------|----------|------|
| **数据隐私** | 基础合规 | 不存储用户源代码（代码操作通过 GitHub API 直接执行），仅存储元数据（工作空间配置、史诗/故事状态） |
...
| 角色 | 权限范围 |
|------|----------|
| **Owner** | 创建/删除工作空间、导入仓库、配置 AI 渠道、提交 PR |
| **Member** | 查看工作空间、参与需求流程、提交 PR（MVP 预留） |
...
4. **Story 2.4：删除工作空间**
   ...
   - 验收标准：
     ...
     - Given 用户确认删除
     - When 提交确认
     - Then 1 秒内从工作空间列表中移除，并将该工作空间标记为软删除（默认查询不可见）
     - Given 用户取消删除
     - When 关闭确认对话框
     - Then 工作空间保持不变
