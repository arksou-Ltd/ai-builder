...
| 类别 | 范围 | 架构意义 |
|------|------|----------|
| 认证与工作空间管理 | FR1-FR9 | GitHub OAuth 集成、多仓库工作空间结构 |
...
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  工作空间设置 → "连接 GitHub 仓库" → GitHub OAuth 授权流程            │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                          │                                              │
│                          ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────┐   │
...
- 仓库操作需要用户在工作空间设置中单独完成 GitHub OAuth 授权
...
async def require_github_authorized(
    ...
    if not account.github_access_token:
        raise HTTPException(
            status_code=403,
            detail="请先在工作空间设置中授权 GitHub 仓库访问",
        )
    return account
```
...
async def get_project_repository(
    ...
    """获取工作空间 Repository（自动注入当前用户 account_id）"""
...
class TestDataIsolation:
    ...
    async def test_user_cannot_access_other_user_workspace(
        ...
        """用户 A 无法访问用户 B 的工作空间"""
        # 用户 A 创建工作空间
        resp_a = await user_a_client.post("/api/workspaces", json={"name": "A的工作空间"})
        workspace_id = resp_a.json()["data"]["id"]

        # 用户 B 尝试访问用户 A 的工作空间
        resp_b = await user_b_client.get(f"/api/workspaces/{workspace_id}")

        # 必须返回 404（而非 403，避免信息泄露）
        assert resp_b.status_code == 404

    async def test_user_cannot_list_other_user_workspaces(
        self, user_a_client: AsyncClient, user_b_client: AsyncClient
    ):
        """用户 A 列表不包含用户 B 的工作空间"""
        # 用户 A 创建工作空间
        await user_a_client.post("/api/workspaces", json={"name": "A的工作空间"})

        # 用户 B 创建工作空间
        await user_b_client.post("/api/workspaces", json={"name": "B的工作空间"})

        # 用户 A 列表查询
        resp_a = await user_a_client.get("/api/workspaces")
        workspaces_a = resp_a.json()["data"]

        # 用户 A 只能看到自己的工作空间
        assert all(p["name"] != "B的工作空间" for p in workspaces_a)
        assert any(p["name"] == "A的工作空间" for p in workspaces_a)

    async def test_user_cannot_update_other_user_workspace(
        self, user_a_client: AsyncClient, user_b_client: AsyncClient
    ):
        """用户 A 无法更新用户 B 的工作空间"""
        # 用户 A 创建工作空间
    ...
    async def test_user_cannot_delete_other_user_workspace(
        self, user_a_client: AsyncClient, user_b_client: AsyncClient
    ):
        """用户 A 无法删除用户 B 的工作空间"""
        # 用户 A 创建工作空间
        ...
        # 用户 B 尝试删除用户 A 的工作空间
        resp_b = await user_b_client.delete(f"/api/workspaces/{workspace_id}")

        # 必须返回 404
        assert resp_b.status_code == 404

        # 验证工作空间仍存在
        resp_a_check = await user_a_client.get(f"/api/workspaces/{workspace_id}")
...
- **认证与工作空间管理（FR1–FR9）**：✅ 覆盖完整
  - 工作空间/仓库模型（ADR-003/ADR-007）覆盖良好
...
| Story 工作空间绑定 | 一个 Story 对应一个仓库 | 简化 PR 管理 |
...
Workspace (1) ──────< Epic (N) ──────< Story (N)
                                        │
                                        └── repository_id → WorkspaceRepository
```
...
| workspace | workspace_workspaces | 工作空间 |
| workspace | workspace_repositories | 工作空间仓库 |
...
| 通信类型 | 协议 | 用途 |
|----------|------|------|
| CRUD 操作 | REST API | 工作空间/Epic/Story 管理 |
