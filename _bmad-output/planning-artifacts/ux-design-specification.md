...
editHistory:
  - date: '2026-02-05'
    changes: '统一认证描述为 Clerk 双层模式、补充登出流程交互设计、补充删除工作空间交互设计'
author: Arksou
---
...
**登录流程：** 访问 → Clerk 登录（邮箱/Google/GitHub Account） → 创建会话 → 添加工作空间 → 配置 AI → 开始工作
...
| 步骤 | 用户行为 | 系统反馈 |
|------|----------|----------|
| 1. 进入工作空间设置 | 点击工作空间卡片的设置图标 | 打开工作空间设置页/面板 |
| 2. 点击删除 | 点击"删除工作空间"按钮 | 弹出 AlertDialog 二次确认 |
| 3. 确认删除 | 点击"确认删除" | 1秒内从工作空间列表移除（软删除归档） |
| 3b. 取消删除 | 点击"取消" | 关闭对话框，工作空间保持不变 |

**删除工作空间 AlertDialog 规范：**
...
<AlertDialog>
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      <AlertDialogTitle>确认删除「{workspaceName}」？</AlertDialogTitle>
      <AlertDialogDescription>
        该操作会将工作空间标记为已删除并从默认列表隐藏。相关数据将保留用于审计与恢复（MVP 暂不提供用户自助恢复入口）。
      </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter>
...
**删除工作空间交互要点：**
- 对话框标题必须包含工作空间名称（避免误删）
- 使用 `variant="destructive"` 的红色按钮
- 取消按钮在左侧，确认删除按钮在右侧
- 删除成功后显示 Toast 提示"工作空间已删除"
...
| **Form + react-hook-form** | AI 配置表单、工作空间设置 |
...
| 场景 | 标题 | 描述 | 操作 |
|------|------|------|------|
| **无工作空间** | 开始您的第一个工作空间 | 创建工作空间并导入 GitHub 仓库 | 「创建工作空间」按钮 |
...
<div className="min-h-screen grid grid-cols-[280px_1fr_360px]">
  {/* 左侧：项目/需求导航 */}
  <nav aria-label="工作空间导航" className="border-r border-slate-700">
    <AppSidebar />
  </nav>
...
<nav aria-label="工作空间导航">
  <ul>
    <li><a href="/workspace/1">工作空间 A</a></li>
  </ul>
</nav>
