...
class TestDependencyChainVersion:
    ...
    def test_arksou_kernel_framework_version(self) -> None:
        """arksou-kernel-framework version must be exactly 0.3.4."""
        version = importlib.metadata.version("arksou-kernel-framework")
        assert version == "0.3.4", (
            f"arksou-kernel-framework version mismatch: expected 0.3.4, got {version}"
        )
...
class TestDependencySourceConfiguration:
    ...
    - arksou-kernel-framework must come from Git SSH source with tag v0.3.4
    ...
    def test_common_kernel_git_ssh_source_in_pyproject(self) -> None:
        ...
        assert framework_source.get("tag") == "v0.3.4", (
            f"arksou-kernel-framework must be pinned to tag v0.3.4, "
    ...
    def test_uv_lock_contains_git_source_with_tag(self) -> None:
        ...
        # Verify tag v0.3.4 is locked in the git URL
        # uv.lock format: ...?tag=v0.3.4#<commit_sha>
        assert "tag=v0.3.4" in content, (
            "arksou-kernel-framework must be pinned to tag v0.3.4 in uv.lock"
        )
